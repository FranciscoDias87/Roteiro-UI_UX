...
export default function ScoreScreen({ playerName, gameMode, questions, answers, totalTime, onRestart }: ScoreScreenProps) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-white border-4 border-[#E0E7FF] rounded-[32px] shadow-2xl overflow-hidden" id="result-hero-card">
        <div className="p-6 sm:p-10 text-center space-y-6">
          <div className="flex justify-center" id="score-icon-container">
            ...
            <div className="relative w-32 h-32 flex items-center justify-center">
              ...
              <div className="absolute text-center">
                <span className="block text-3xl font-black text-slate-800">{correctCount}</span>
                <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest">de {totalQuestions}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full border text-xs font-black uppercase tracking-widest bg-[#EEF2FF] border-[#6366F1] text-[#6366F1]`} id="performance-badge">
              {performance.title}
            </div>
            <p className="text-sm font-medium text-slate-500 max-w-md mx-auto leading-relaxed mt-2 p-1.5">
              {performance.feedback}
            </p>
          </div>

          {/* Quick Metrics */}
          <div className="grid grid-cols-3 gap-3 border-t border-b border-indigo-50/60 py-5 my-2 max-w-md mx-auto" id="score-metrics">
            <div className="text-center">
              <span className="block text-[10px] text-slate-400 font-extrabold uppercase tracking-wide">Aproveitamento</span>
              <span className="text-base sm:text-lg font-black text-slate-800 font-mono">{percentage}%</span>
            </div>
            <div className="text-center border-l border-slate-100">
              <span className="block text-[10px] text-slate-400 font-extrabold uppercase tracking-wide">Tempo Total</span>
              <span className="text-base sm:text-lg font-black text-slate-800 font-mono flex items-center justify-center gap-1">
                <Clock className="w-4 h-4 text-slate-450" /> {formatTimeMinutes(totalTime)}
              </span>
            </div>
            <div className="text-center border-l border-slate-100">
              <span className="block text-[10px] text-slate-400 font-extrabold uppercase tracking-wide">Modo</span>
              <span className="text-xs font-black text-[#6366F1] bg-[#EEF2FF] px-2.5 py-0.5 rounded-full inline-block mt-0.5">
          ...
          <div className="pt-2">
            <button
              onClick={onRestart}
              id="play-again-btn"
              className="inline-flex items-center justify-center gap-2 px-8 py-3 bg-[#6366F1] hover:bg-[#5053db] cursor-pointer text-white font-black rounded-xl text-sm transition-all shadow-[0_4px_0_#4338CA] active:translate-y-[2px] active:shadow-[0_2px_0_#4338CA]"
            >
          ...
          return (
            <div
              ...
              className={`bg-white border-2 rounded-2xl overflow-hidden transition-all duration-200 ${
                isCorrect ? 'border-emerald-200' : 'border-rose-250'
              }`}
              id={`review-question-row-${question.id}`}
            >
              <div
                onClick={() => toggleExplanation(question.id)}
                className="p-5 flex items-start gap-4 cursor-pointer hover:bg-slate-50/50 select-none"
              >
                {/* Result stamp */}
                <span
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    <span className="text-[10px] uppercase font-black tracking-wider text-slate-400 bg-slate-100 px-2 py-0.5 rounded">
                      Questão {index + 1}
                    </span>
                    <span className="text-[10px] uppercase font-black tracking-wider text-[#6366F1] bg-[#EEF2FF] px-2 py-0.5 rounded">
                      {question.topic}
                    </span>
                  </div>
                  <p className="text-sm sm:text-base font-bold text-slate-800 line-clamp-2 md:line-clamp-none pr-1">
              ...
              {/* Expansion block */}
              {isExpanded && (
                <div className="px-5 pb-5 pt-1 border-t border-slate-50 space-y-4 bg-slate-50/30">
                  {/* Entire Question Text for full context */}
                  <div className="text-sm text-slate-705 leading-relaxed bg-slate-50 p-4 rounded-xl border border-slate-150 whitespace-pre-line">
                    <p className="font-extrabold text-[#6366F1] text-[10px] uppercase tracking-wider mb-2">Conceito Completo:</p>
                    {question.text}
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-2.5">
                    {Object.entries(question.options).map(([key, value]) => {
                      ...
                      return (
                        <div
                          key={key}
                          className={`p-3 rounded-xl border-2 text-xs sm:text-sm flex gap-3.5 items-start ${
                            isCorrectOption
                        ...
                        >
                          <span
                            className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 text-[10px] font-black border ${
                              isCorrectOption
                          ...
                          <span className="flex-1 mt-0.5">{value}</span>
                          {isCorrectOption && (
                            <span className="text-[9px] font-black bg-emerald-100 text-emerald-800 px-2.5 py-1 rounded shrink-0 uppercase tracking-widest self-center">
                              Gabarito
                            </span>
                          )}
                          {isStudentOption && !isCorrectOption && (
                            <span className="text-[9px] font-black bg-rose-100 text-rose-700 px-2.5 py-1 rounded shrink-0 uppercase tracking-widest self-center">
                              Seu Erro
                            </span>
                          )}
                        </div>
                  ...
                  <div className="bg-[#FACC15] text-[#854D0E] border border-amber-305 rounded-xl p-4 sm:p-5">
                    <h4 className="font-black text-[#854D0E] text-xs sm:text-sm mb-1.5 flex items-center gap-1.5 uppercase tracking-wider">
